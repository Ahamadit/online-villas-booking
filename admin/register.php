...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($useremail_err) && empty($username_err) && empty($password_err) && empty($confirm_password_err)) {
        ...
        $sql = "INSERT INTO users (useremail, username, password, token, isvalid) VALUES (?, ?, ?, ?, ?)";

        if ($stmt = mysqli_prepare($link, $sql)) {
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssss", $param_useremail, $param_username, $param_password, $param_token, $isvalid);
